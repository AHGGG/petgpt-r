...
export class PluginLoader implements IPluginLoader {
  ...
  registerPlugin(name: string, plugin?: IPetPlugin): void {
    ...
    try {
      ...
      if (!plugin) {
        if (
          this.ctx.db.get(`petPlugins.${name}`) === true ||
          this.ctx.db.get(`petPlugins.${name}`) === undefined
        ) {
      ...
      this.enabledPluginList = this.enabledPluginList.filter(
  ...
  async getPlugin(name: string): Promise<IPetPluginInterface | undefined> {
    ...
    // const winPluginAbsolutePath = 'file://' + this.resolvePlugin(name);
    const winPluginAbsolutePath =
      process.env.NODE_ENV === 'development'
        ? this.resolvePlugin(name)
        : `file://${this.resolvePlugin(name)}`;

    // 动态加载 ==> export default的东西
    // eslint-disable-next-line global-require,import/no-dynamic-require
    const model = require(winPluginAbsolutePath);
    // let { default: obj } = await import('C:\\Users\\15275\\AppData\\Roaming\\petgpt-r\\node_modules\\petgpt-plugin-template\\dist');
    // let { default: obj } = require('C:\\Users\\15275\\AppData\\Roaming\\petgpt-r\\node_modules\\petgpt-plugin-template\\dist');
    // const { default: pluginFunction } = require(this.resolvePlugin(name));
    // .then((m) => m)
    // .catch((e) => {
    //   logger.info(`plugin import error: `, e);
    // });
    const plugin: IPetPluginInterface = model.default(this.ctx); // 动态引入后调用函数，返回IPetPluginInterface对象
