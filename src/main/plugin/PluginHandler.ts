...
import { ILogger, IPluginHandlerResult } from './share/types';
import { getNormalPluginName, getProcessPluginName } from './common';
import logger from '../utils/logger';
import { DBList } from '../../common/enum';
import { IPluginProcessResult } from '../../common/types';
...
export async function execCommand(
  ...
  const registry =
    options.registry || dbMap?.get(DBList.Config_DB)?.get('settings.registry');
  const proxy =
    options.proxy || dbMap?.get(DBList.Config_DB)?.get('settings.proxy');
  ...
  if (fullNameList.length > 0) {
    ...
    if (!result.code) {
      pkgNameList.forEach((pluginName: string) => {
        pluginLoader.registerPlugin(pluginName);
      });
