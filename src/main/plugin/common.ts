...
import { IPluginNameType } from '../../common/types';
import { ILogger } from './share/types';
 ...
 * @param scope
 */
export const removePluginVersion = (
  nameOrPath: string,
  scope: boolean = false,
): string => {
...
export const getPluginNameType = (name: string): IPluginNameType => {
  ...
  if (name.startsWith('petgpt-plugin-')) {
    return 'normal';
  }
 ...
 * @param logger
 */
export const getNormalPluginName = (
  nameOrPath: string,
 ...
 * @param logger
 */
export const getProcessPluginName = (
  nameOrPath: string,
  logger: ILogger = console,
): string => {
  const pluginNameType = getPluginNameType(nameOrPath);
