...
import { IPluginConfig } from '../../main/plugin/share/types';
import { PluginInfo, Progress, Rule } from '../../common/types';
...
function Setting() {
  ...
  async function getPluginsNameList() {
    ...
    const pluginInfoList = await window.electron.ipcRenderer.invoke(
  ...
  useMount(async () => {
    ...
    window.electron.ipcRenderer.on('installSuccess', (event, data) => {
      logger(
        `installSuccess: `,
        data,
        ` pluginsConfigList:`,
    ...
    window.electron.ipcRenderer.on('uninstallSuccess', (event, data) => {
      logger(
        `uninstallSuccess: `,
        data,
        ` pluginsConfigList:`,
    ...
    window.electron.ipcRenderer.on('updateSuccess', (event, data) => {
    ...
    window.electron.ipcRenderer.on('failedProgress', () => {
      upOrDeleteProgress.forEach((progress, index) => {
        if (progress.percentage > 0) {
