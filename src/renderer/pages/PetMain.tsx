...
function useMove() {
  ...
  function handleMouseMove(e: MouseEvent) {
    ...
    if (dragging) {
      ...
      window.electron.ipcRenderer.sendMessage('Set_Main_Window_Pos', {
        x: xLoc,
        y: yLoc,
...
function PetMain() {
  ...
  useMove();
  useEffect(() => {
    window.electron.ipcRenderer.invoke('platform').then((p) => setPlatform(p));
    window.electron.ipcRenderer.invoke('publicDir').then((dir) => {
      setPublicDir(dir);
  ...
  useEffect(() => {
    window.electron.ipcRenderer.on('Change_Image_Replay', () => {
    ...
    return () => {
      window.electron.ipcRenderer.removeAllListeners('Change_Image_Replay');
    };
  });
